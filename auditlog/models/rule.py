...
class AuditlogRule(models.Model):
    ...
    def _create_log_line_on_read(
        ...
        for field_name in fields_list:
            if field_name in fields_to_exclude:
                continue
            # Skip JSON dict fields (e.g. partner_data)
            val = read_values.get(log.res_id, {}).get(field_name)
            if self._is_json_field_value(val):
                continue
    ...
    @staticmethod
    def _is_json_field_value(value):
        """Check if a value is a JSON dict string (e.g. partner_data)."""
    ...
    def _create_log_line_on_write(
        ...
        for field_name in fields_list:
            if field_name in fields_to_exclude:
                continue
            # Skip JSON dict fields (e.g. partner_data) since individual
            # fields are already logged separately
            old_val = old_values.get(log.res_id, {}).get(field_name)
            new_val = new_values.get(log.res_id, {}).get(field_name)
            if self._is_json_field_value(old_val) or self._is_json_field_value(new_val):
                continue
            field = self._get_field(log.model_id, field_name)
            # not all fields have an ir.models.field entry (ie. related fields)
            if field:
    ...
    def _create_log_line_on_create(
        ...
        for field_name in fields_list:
            if field_name in fields_to_exclude:
                continue
            # Skip JSON dict fields (e.g. partner_data)
            val = new_values.get(log.res_id, {}).get(field_name)
            if self._is_json_field_value(val):
                continue
            field = self._get_field(log.model_id, field_name)
            # not all fields have an ir.models.field entry (ie. related fields)
